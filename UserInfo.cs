...
namespace DAL
{
    ...
    {
        using (IDConnection cn=new MySqlConnection(cns))
        {
    ...
    public int Add(Model.UserInfo user)
    {
        using (IDbConnection cn= new MySqlConnection(cns))
        {
            ...
            return cn.Query<Model.UserInfo>(sql,user);
        }
    }
    public int Update(Model.UserInfo user)
    {
        using (IDbConnection cn = new MySqlConnection(cns))
        {
            string sql = "update userinfo set password=@passWord,qq=@qq,email=@email,type=@type,userimg=@iserImg where username=@userName";
            return cn.Query<Model.UserInfo>(sql);
        }
    }
